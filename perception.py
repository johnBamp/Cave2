...
def cast_and_update(cfg: Config, state):
    seen_empty = set()
    seen_wall = set()
    newly_seen = 0

    half_fov = math.radians(cfg.fov_deg) / 2.0
    start_angle = state.player_angle - half_fov
    end_angle = state.player_angle + half_fov
    ...
    for (cx, cy) in seen_empty:
        if not state.ever_seen[cx][cy]:
            newly_seen += 1
        state.subjective[cx][cy] += (0.0 - state.subjective[cx][cy]) * cfg.empty_alpha
        state.ever_seen[cx][cy] = True

    for (cx, cy) in seen_wall:
        if not state.ever_seen[cx][cy]:
            newly_seen += 1
        state.subjective[cx][cy] += (1.0 - state.subjective[cx][cy]) * cfg.wall_alpha
        state.ever_seen[cx][cy] = True

    return seen_empty | seen_wall, newly_seen


def infer_enclosed_voids(cfg: Config, state) -> None:
    ...
    for gx in range(cfg.tiles_x):
        for gy in range(cfg.tiles_y):
            if is_known_open(cfg, state, gx, gy) and not visited[gx][gy]:
                visited[gx][gy] = True
                q.append((gx, gy))
    ...
    while q:
        ...
        for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            nx, ny = x + dx, y + dy
            if in_bounds_cell(cfg, nx, ny) and not visited[nx][ny] and not is_confirmed_wall(cfg, state, nx, ny):
                visited[nx][ny] = True
                q.append((nx, ny))

    for gx in range(1, cfg.tiles_x - 1):
        for gy in range(1, cfg.tiles_y - 1):
            if is_confirmed_wall(cfg, state, gx, gy) or visited[gx][gy]:
                continue
            region = []
            dq = deque()
            dq.append((gx, gy))
            ...
            if len(region) >= cfg.min_enclosed_size:
                for (x, y) in region:
                    state.subjective[x][y] = 1.0
                    state.ever_seen[x][y] = True


def observe_local_3x3(cfg: Config, state, current_cell, game_time: float) -> int:
    newly_seen = 0
    ...
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
            ...
            if not state.ever_seen[cx][cy]:
                newly_seen += 1
            if state.objective[cx][cy] == 1:
                state.subjective[cx][cy] = 1.0
            else:
                state.subjective[cx][cy] = 0.0
            state.ever_seen[cx][cy] = True
